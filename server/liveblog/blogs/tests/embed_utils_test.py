...
class EmbedUtilsTestCase(TestCase):
    def test_get_setting_value(self):
        ...
        self.assertEqual(
            get_setting_value(settings, "blog-title", "font-weight"), "normal"
        )

        # also check the linked group values
        primary_font_mukta = get_setting_value(
            settings, "blog-title", "font-family", linked_to_group="typography"
        )
    ...
    def test_build_css_selector(self):
        ...
        selector_with_child_tag = build_css_selector(
            group_selector, style_option_with_tag
        )
